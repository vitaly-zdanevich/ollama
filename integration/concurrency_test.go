...
func TestIntegrationConcurrentPredictOrcaMini(t *testing.T) {
	...
	vram := envconfig.MaxVRAM()
	if vram < 4*format.GibiByte {
...
func TestMultiModelStress(t *testing.T) {
	vram := envconfig.MaxVRAM()
	if vram == 0 {
		t.Skip("skipping test; OLLAMA_MAX_VRAM not set")
	}
	...
	switch {
	case vram < 10000*format.MebiByte:
		slog.Info("selecting small models")
		chosenModels = smallModels
	// case vram < 30000*format.MebiByte:
	default:
